...
class GeneAggregator(nn.Module):
    ...
    def forward(self, shard_embeds):
        ...
        return self.attention(shard_embeds, shard_embeds, shard_embeds).squeeze(1)


class Varformer(nn.Module):
    def __init__(self, num_mutations, max_seq_length, d_model, nhead, num_layers):
        super(Varformer, self).__init__()

        dim_feedforward = int(d_model // 2)
        self.variant_embedding = VariantEmbedding(num_mutations, max_seq_length, d_model)
...
class ShardedVarformer(nn.Module):
    def __init__(self, max_seq_len, num_muts, d_model=128, nhead=2, num_encoder_layers=2):
    ...
    def forward(self, pathogenicity, position, mutation, mask):
        ...
        x = torch.cat([pat_embed, pos_embed, mut_embed], dim=-1)
