"""Shim — most content moved to varformer.data. Delete in Phase 8.

What moved:
  - GeneCharacterisationPreprocessor -> varformer.data.features.gc
  - GeneOntologyPreprocessor -> varformer.data.features.go
  - PopulationVariantPreprocessor -> varformer.data.features.variants
  - extract_pvc_features -> varformer.data.features.variants
  - ModelPreprocessorEval -> varformer.data.loaders
  - ModelPreprocessorInference -> varformer.data.loaders

What is still here:
  - LogisticRegressionPreprocessor (moves to paper/baselines/ in Phase 5)
  - module-level imports kept for downstream compatibility

Dead classes removed (had double-underscore prefix, verified no callers):
  - __WildtypeLoader (was lines ~2673-2799)
  - __MissenseVariantPreprocessor (was lines ~2800-3153)
"""
import os
import gc
import warnings
...
# Re-exports from varformer.data
...
# LogisticRegressionPreprocessor stays here for now (Phase 5 moves it to paper/baselines/)
